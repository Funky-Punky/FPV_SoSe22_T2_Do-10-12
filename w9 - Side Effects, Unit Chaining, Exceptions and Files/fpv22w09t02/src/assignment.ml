  ...
  if count = 0 then []
    ...
    try
      ...
      | [ course_s; grade_s ] -> (
          ...
          with Failure _ ->
            raise (My_Corrupt_database_file "A Field in a Grade has the wrong format"))
      | _ -> raise (My_Corrupt_database_file "Grade line has wrong count of fields")
    with End_of_file ->
      raise (My_Corrupt_database_file "GradeCount for Student was to large")
...
let read_student_from_channel channel =
  ...
  | [ first_name; last_name; id_s; semester_s; gradecount_s ] -> (
      ...
      with Failure _ ->
        raise (My_Corrupt_database_file "A Field in a Student has the wrong format"))
  | _ -> raise (My_Corrupt_database_file "Student line has wrong count of fields")
...
let my_load_db filename =
  ...
  try
    ...
    close_in channel;
    db
  with My_Corrupt_database_file msg ->
    close_in channel;
    raise (My_Corrupt_database_file msg)
